    ...
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: "var(--bg-main)" }}>
      <div className="w-full max-w-sm animate-fade-in-up">
        <div className="text-center mb-8">
          ...
          <p className="text-sm mt-1" style={{ color: "var(--text-muted)" }}>تسجيل دخول الأدمن</p>
        </div>
        ...
        <p className="text-center text-xs mt-4" style={{ color: "var(--text-muted)" }}>هذه الصفحة للأدمن فقط</p>
        {onCancel && <button onClick={onCancel} className="w-full mt-3 py-2 rounded-xl text-sm font-semibold transition" style={{ color: "var(--text-secondary)", border: "1px solid var(--border)" }}>← العودة</button>}
      </div>
    </div>
  );
}
